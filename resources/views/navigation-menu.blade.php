<div>
  <nav class="fixed top-0 z-40 w-full border-b border-gray-200 bg-white dark:bg-gray-800">
      <div class="px-3 py-3 lg:px-5 lg:pl-3">
        <div class="flex items-center justify-between">
          <div class="flex items-center justify-start">
            <button data-drawer-target="logo-sidebar" data-drawer-toggle="logo-sidebar" aria-controls="logo-sidebar" type="button" class="inline-flex items-center p-2 text-sm text-gray-500 bg-transparent rounded-lg sm:hidden">
  ...
  <aside id="logo-sidebar" class="fixed top-0 left-0 z-50 w-64 pt-5 h-screen transition-transform -translate-x-full border-r sm:translate-x-0 bg-gray-800 border-gray-700" aria-label="Sidebar">
      <a href="{{ route('welcome') }}" class="flex ml-2 md:mr-24">
        <span class="self-center ml-2 text-xl font-semibold sm:text-2xl whitespace-nowrap text-white dark:text-white">Pangkalan LPG</span>
      </a>
      <div class="h-full pt-9 px-3 pb-4 overflow-y-auto bg-gray-800">
        <ul class="space-y-2 font-medium">
            ...
            <li>
              <a href="{{ route('transactions.create') }}" class="{{ Route::is('transactions.create') ? 'flex items-center p-2 rounded-lg text-white bg-gray-700' : 'flex items-center p-2 rounded-lg text-white hover:bg-gray-700'}}">
            ...
            <li>
              <button type="button" class="flex items-center w-full p-2 text-white transition duration-75 rounded-lg group hover:bg-gray-700" aria-controls="dropdown-example" data-collapse-toggle="dropdown-example">
                    <svg aria-hidden="true" class="flex-shrink-0 w-6 h-6 text-gray-400 transition duration-75 dark:text-gray-400 dark:group-hover:text-white" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              ...
              <ul id="dropdown-example" class="hidden py-2 space-y-2" style="{{ request()->routeIs('transactions.purchase', 'transactions.selling', 'search') ? 'display: block;' : '' }}">
                    <li>
                      <a href="{{ route('transactions.purchase') }}" class="flex items-center w-full p-2 text-white transition duration-75 rounded-lg pl-11 group hover:bg-gray-700 {{ request()->routeIs('transactions.purchase') ? 'bg-gray-700' : 'hover:bg-gray-700' }}">Pembelian</a>
                    </li>
                    <li>
                      <a href="{{ route('transactions.selling') }}" class="flex items-center w-full p-2 text-white transition duration-75 rounded-lg pl-11 group hover:bg-gray-700 {{ request()->routeIs('transactions.selling') ? 'bg-gray-700' : 'hover:bg-gray-700' }}">Penjualan</a>
                  </li>
              </ul>
            </li>
            <li>
